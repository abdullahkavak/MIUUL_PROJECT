...
sql_conn = pyodbc.connect('Driver={Sql Server};'
                         'Server=DESKTOP-AJVAAHS\SQLEXPRESS;'
                         'Database:MIUUL;'
                         'TrustedConnection:True;')
query = "SELECT  *  FROM [MIUUL].[dbo].[train]"
train = pd.read_sql(query, sql_conn)

train.head(3)

query = "SELECT  *  FROM [MIUUL].[dbo].[test]"
test = pd.read_sql(query, sql_conn)
...
for name in ['MSSubClass', 'OverallCond', 'YrSold', 'MoSold']:
    ...
    for col_name in label_encoder_col_names:
        ...
        datasets_no_missing['TotalSF'] = datasets_no_missing['TotalBsmtSF'] + datasets_no_missing['1stFlrSF'] + \
                                         datasets_no_missing['2ndFlrSF']
...
cursor.execute("truncate table MIUUL.dbo.predicted_results" )
for index,row in submision.iterrows():
    cursor.execute("INSERT INTO MIUUL.dbo.predicted_results(prediction_date,Id,SalePrice) values (?, ?, ?)",now.strftime("%d/%m/%Y %H:%M:%S"), row['Id'], row['SalePrice'] )
    sql_conn.commit()
cursor.close()
sql_conn.close()
